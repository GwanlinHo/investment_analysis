...
- **2026-07-29 (v2.7)**:
  - **事件時序鐵律 (Event-Timing Integrity)**:
    ...
    - **修正**：`CLAUDE.md` 新聞規範與 AI 分析反幻覺協定各加入強制條款：凡排程性事件（央行決策、財報、經濟數據）必須確認官方發布時間早於報告生成時間（含美東→台灣時區換算）才可寫成已發生，否則一律前瞻語氣並註明揭曉時間；把預期改寫成事實或捏造來源即整則棄用。
